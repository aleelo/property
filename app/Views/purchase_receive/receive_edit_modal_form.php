...
<div class="modal-body clearfix">
    ...
    <div class="container-fluid">
        <div class="form-group mb0 d-flex justify-content-between mt-4">
            <h4 class="fw-bold mb-1 mt-4 mb0">Purchase Receive Details:</h4>
            <button class="btn btn-primary" id='add_remaining_items'><span data-feather="plus" class="icon-16"></span> Add remaining Items</button>
        ...
        <div class="form-group p-3" style="clear: both;">
            <div class="row">
                <table class="table table-bordered" id="add_items_table">
                    ...
                    <tbody>
                        <?php 
                        //foreach ($receive_details as $index => $order) {?>
                            <!-- <tr>
...
<div class="modal-footer">
    ...
    <button type="submit" class="btn btn-primary"><span data-feather="check-circle" class="icon-16"></span> <?php echo app_lang('update_purchase_receive'); ?></button>
</div>
<?php echo form_close(); ?>
...
<script type="text/javascript">
    ...
    function getFooterTotal(total=0.0,el){
        ...
        curTotal = localStorage.getItem('curTotal1');
        total = curTotal - t;
    ...
    $(document).ready(function () {
        ...
        $('#add_remaining_items').on('click',function(e){

            e.preventDefault();

                //get items detail:
                addRemainingOrderItems();
                $('#add_remaining_items').hide();
            });
            ...
            function getReceiveItemsDetail(){
                ...
                let remainder = 0;
                $.ajax({
                    url: 'purchase_receive/receive_items_json/'+$('#order_id').val(),
                    cache: false,
                    type: 'GET',
                    success: function (data) {
                        ...
                        if(data.length > 0 && data[0].id != null){
                            for(let i=0;i< data.length;i++){
                                ...
                                $.ajax({
                                    url: 'purchase_receive/get_item_details_json/'+data[i].item_id + '/'+data[i].purchase_order_id,
                                    cache: false,
                                    type: 'GET',
                                    success: function (res) {
                                        ...
                                        $('#add_items_table tbody').append(
                                            ...
                                            "<td>" + k + "</td>"+
                                                ...
                                                "<td style='text-align: center;'><input type='text' class='form-control' style='background-color: #fafdff;text-align:center;margin-top: 15px;' value='" + data[i].quantity + "' oninput='calculate_total(this)' "+
                                                "data-rule-required data-msg-required='This field is required.' id='quantity_" + k + "' placeholder='Quantity'  name='quantity[]' style='border: 0;'><span style='font-size: 12px;color:"+color+"'>"+remainder+"</span></td>"+
            ...
            function addRemainingOrderItems(){
                                
                $.ajax({
                    url: 'purchase_receive/order_items_json/'+$('#purchase_order_id').val(),
                    cache: false,
                    type: 'GET',
                    success: function (data) {
                        ...
                        if(data.length > 0 && data[0].id != null){
                            for(let i=0;i< data.length;i++){
                                ...
                                $('#add_items_table tbody #totalRow').before(
                                    "<tr class='' style='vertical-align: middle;'>"+
                                    "<td>" + k + "</td>"+
                                        "<td><input type='hidden' class='form-control'  value='" + data[i].item_id + "'  id='item_id_" + k + "' placeholder='item Name' name='item_id[]'>"+
                                        "<input type='text' class='form-control' style='background-color: #dddddd;' value='" + data[i].name + "' data-rule-required data-msg-required='This field is required.' id='item_name_" + k + "' placeholder='item Name' name='item_name[]' readonly style='border: 0'></td>"+
                                        "<td><textarea class='form-control' style='background-color: #fafdff;' id='description_" + k + "' placeholder='description'  name='description[]' style='border: 0'>" + data[i].description + "</textarea></td>"+
                                        // "<td><input type='text' class='form-control' style='background-color: #fafdff' value='" + data[i].unit_type + "' data-rule-required data-msg-required='This field is required.' id='unit_type_" + k + "' placeholder='Unit Type'  name='unit_type[]' readonly style='border: 0'></td>"+
                                        "<td style='text-align: center;'><input type='text' class='form-control' style='background-color: #fafdff;text-align:center' value='" + data[i].quantity + "' oninput='calculate_total(this)' "+
                                        "data-rule-required data-msg-required='This field is required.' id='quantity_" + k + "' placeholder='Quantity'  name='quantity[]' style='border: 0'><span style='font-size: 12px;'>"+remainder+"</span></td>"+
                                        "<td><input type='text' class='form-control' style='background-color: #fafdff;' value='" + data[i].price + "' data-rule-required data-msg-required='This field is required.' id='price_" + k + "' placeholder='Price'  name='price[]' style='border: 0'></td>"+
                                        "<td><input type='text' class='form-control' style='background-color: #fafdff;' value='" + parseFloat(data[i].total).toFixed(2) + "' data-rule-required data-msg-required='This field is required.' id='total_" + k + "' placeholder='Total'  name='total[]' style='border: 0'></td>"+
                                        "<td style='text-align: center;'>" + actions + "</td>"+
                                    "</tr>"
                            ...
                            var oldTotal =  parseFloat(localStorage.getItem('curTotal1'));                       

                            // alert(total);
                            var newTotal = oldTotal + total;


                            $('#add_items_table tbody #footer_total').html( newTotal.toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2, style: 'currency', currency: 'USD'}));

                            localStorage.setItem('curTotal1', newTotal);
