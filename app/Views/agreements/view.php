        ...
        <div class="row clients-view-button">
            <div class="col-md-12">
                <div class="page-title clearfix no-border no-border-top-radius no-bg">
                    <h1 class="pl0">
                        <?php echo 'Agreement Details' . " - " . $agreement_info->titleDeedNo ?>
                        <span id="star-mark">
                            <?php
                            if ($is_starred) {
                                echo view('agreements/star/starred', array("client_id" => $agreement_info->id));
                            } else {
                                echo view('agreements/star/not_starred', array("client_id" => $agreement_info->id));
                            }
                            ?>
                        </span>

                        <?php if ($agreement_info->lead_status_id) { ?>
                            <?php $lead_information = app_lang("past_lead_information") . "<br />"; ?>
                            <?php if ($agreement_info->created_date) { ?>
                                <?php $lead_information .= app_lang("lead_created_at") . ": " . format_to_date($agreement_info->created_date, false) . "<br />"; ?>
                            <?php } ?>
                            <?php if ($agreement_info->client_migration_date && is_date_exists($agreement_info->client_migration_date)) { ?>
                                <?php $lead_information .= app_lang("migrated_to_client_at") . ": " . format_to_date($agreement_info->client_migration_date, false) . "<br />"; ?>
                            <?php } ?>
                            <?php if ($agreement_info->last_lead_status) { ?>
                                <?php $lead_information .= app_lang("last_status") . ": " . $agreement_info->last_lead_status . "<br />"; ?>
                            <?php } ?>
                            <?php if ($agreement_info->owner_id) { ?>
                                <?php $lead_information .= app_lang("owner") . ": " . $agreement_info->owner_name; ?>
                            <?php } ?>
                    ...
                    <?php if (can_access_reminders_module()) { ?>
                        <div class="title-button-group mr0 clients-view">
                            <?php echo modal_anchor(get_uri("events/reminders"), "<i data-feather='clock' class='icon-16'></i> " . app_lang('reminders'), array("class" => "btn btn-default mr0", "id" => "reminder-icon", "data-post-client_id" => $agreement_info->id, "data-post-reminder_view_type" => "client", "title" => app_lang('reminders') . " (" . app_lang('private') . ")")); ?>
                        </div>
                    <?php } ?>
                </div>

                <div>
                    <?php// echo view("agreements/info_widgets/index"); ?>
                </div>

                <ul id="client-tabs" data-bs-toggle="ajax-tab" class="nav nav-tabs scrollable-tabs" role="tablist">
                    
                    <li><a  role="presentation" data-bs-toggle="tab" href="<?php echo_uri("agreements/company_info_tab/" . $agreement_info->id); ?>" data-bs-target="#client-info"> <?php echo 'Agreement Info'; ?></a></li>
                    <li><a  role="presentation" data-bs-toggle="tab" href="<?php echo_uri("agreements/files/" . $agreement_info->id); ?>" data-bs-target="#client-files"><?php echo app_lang('files'); ?></a></li>
\                   

                    <?php
                    $hook_tabs = array();
                    $hook_tabs = app_hooks()->apply_filters('app_filter_client_details_ajax_tab', $hook_tabs, $agreement_info->id);
