...
class Properties_owner_log_model extends Crud_model {

    protected $table = null;

    function __construct() {
        $this->table = 'properties_owner_log';
    ...
    function get_details($options = array()) {
        ...
        if ($id) {
            $where .= " AND $agreements_table.id=$id";
        }
        ...
        if ($leads_only) {
            ...
            $where .= " AND $agreements_table.is_lead=1";
        }

        $status = $this->_get_clean_value($options, "status");
        if ($status) {
            $where .= " AND $agreements_table.lead_status_id='$status'";
        }

        $source = $this->_get_clean_value($options, "source");
        if ($source) {
            $where .= " AND $agreements_table.lead_source_id='$source'";
        }

        $owner_id = $this->_get_clean_value($options, "owner_id");
        if ($owner_id) {
            $where .= " AND $agreements_table.owner_id=$owner_id";
        }

        $created_by = $this->_get_clean_value($options, "created_by");
        if ($created_by) {
            $where .= " AND $agreements_table.created_by=$created_by";
        }
        ...
        if ($show_own_clients_only_user_id) {
            $where .= " AND $agreements_table.section_head_id=$show_own_clients_only_user_id";
        }


        $quick_filter = $this->_get_clean_value($options, "quick_filter");
        if ($quick_filter) {
            $where .= $this->make_quick_filter_query($quick_filter, $agreements_table, $projects_table, $invoices_table, $invoice_payments_table, $estimates_table, $estimate_requests_table, $tickets_table, $orders_table, $proposals_table);
        }

        $start_date = $this->_get_clean_value($options, "start_date");
        if ($start_date) {
            $where .= " AND DATE($agreements_table.created_date)>='$start_date'";
        }
        $end_date = $this->_get_clean_value($options, "end_date");
        if ($end_date) {
            $where .= " AND DATE($agreements_table.created_date)<='$end_date'";
        }
        ...
        $where .= $this->prepare_allowed_client_groups_query($agreements_table, $client_groups);

        //prepare custom fild binding query
        $custom_fields = get_array_value($options, "custom_fields");
        $custom_field_filter = get_array_value($options, "custom_field_filter");
        $custom_field_query_info = $this->prepare_custom_field_query_string($custom_field_type, $custom_fields, $agreements_table, $custom_field_filter);
        ...
        if ($search_by) {
            ...
            if ($leads_only) {
                ...
                $where .= $this->get_custom_field_search_query($agreements_table, "leads", $search_by);
            } else {
                $where .= $this->get_custom_field_search_query($agreements_table, "clients", $search_by);
            }
