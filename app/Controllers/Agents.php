...
class Agents extends Security_Controller {
    ...
    function index($tab = "") {
        ...
        // $this->check_module_availability("module_section");
        ...
        return $this->template->rander("agents/index", $view_data);
    }
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->Agents_model->get_one($Sections_id);
        ...
        return $this->template->view('agents/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        if ($property_id) {
            $property_info = $this->Agents_model->get_one($property_id);
        ...
        $save_id = $this->Agents_model->ci_save($data, $property_id);
        ...
        if ($save_id) {
            ...
            if(!$property_id){
                ...
                $partner = $this->Agents_model->get_details($options)->getRow();
    ...
    function delete() {
        ...
        if ($this->Agents_model->delete($id)) {
    ...
    function list_data() {
        ...
        $result = $this->Agents_model->get_details($all_options);
    ...
    private function _row_data($id) {
        ...
        $data = $this->Agents_model->get_details($options)->getRow();
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $row_data = array(
            $data->id,
            anchor(get_uri("agents/view/" . $data->id), $data->titleDeedNo),
            $data->service_name,
            $data->owner_name,
        ...
        $row_data[] = modal_anchor(get_uri("agents/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_property'), "data-post-id" => $data->id))
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_property'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("agents/delete"), "data-action" => "delete-confirmation"));
    ...
    public function get_owners_by_property_id()
    {
        ...
        $owners = $this->Agents_model->get_owners_by_property($property_id);
    ...
    function view($Sections_id = 0, $tab = "") {
        ...
        if ($Sections_id) {
            ...
            $properties_info = $this->Agents_model->get_details($options)->getRow();
            if ($properties_info && !$properties_info->is_lead) {
                ...
                return $this->template->rander("agents/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function add_remove_star($client_id, $type = "add") {
        if ($client_id) {
            ...
            if ($type === "add") {
                ...
                return $this->template->view('agents/star/starred', $view_data);
            } else {
                $this->Clients_model->add_remove_star($client_id, $this->login_user->id, $type = "remove");
                return $this->template->view('agents/star/not_starred', $view_data);
            }
        }
    }
    ...
    function show_my_starred_clients() {
        ...
        return $this->template->view('agents/star/clients_list', $view_data);
    }
    ...
    function projects($client_id) {
        ...
        return $this->template->view("agents/projects/index", $view_data);
    }
    ...
    function payments($client_id) {
        ...
        if ($client_id) {
            ...
            $view_data['client_id'] = clean_data($client_id);
            return $this->template->view("agents/payments/index", $view_data);
        }
    }
    ...
    function tickets($client_id) {
        ...
        if ($client_id) {
            ...
            return $this->template->view("agents/tickets/index", $view_data);
        }
    }
    ...
    function invoices($client_id) {
        ...
        if ($client_id) {
            ...
            return $this->template->view("agents/invoices/index", $view_data);
        }
    }
    ...
    function estimates($client_id) {
        ...
        if ($client_id) {
            ...
            return $this->template->view("agents/estimates/estimates", $view_data);
        }
    }
    ...
    function orders($client_id) {
        ...
        if ($client_id) {
            ...
            return $this->template->view("agents/orders/orders", $view_data);
        }
    }
    ...
    function estimate_requests($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = clean_data($client_id);
            return $this->template->view("agents/estimates/estimate_requests", $view_data);
        }
    }
    ...
    function notes($client_id) {
        ...
        if ($client_id) {
            $view_data['client_id'] = clean_data($client_id);
            return $this->template->view("agents/notes/index", $view_data);
        }
    }
    ...
    function files($property_id, $view_type = "") {
        ...
        if ($view_type == "page_view") {
            ...
            return $this->template->rander("agents/files/index", $view_data);
        } else {
            return $this->template->view("agents/files/index", $view_data);
        }
    }
    ...
    function file_modal_form() {
        ...
        return $this->template->view('agents/files/modal_form', $view_data);
    }
    ...
    private function _make_file_row($data) {
        ...
        $description = "<div class='float-start'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("agents/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("agents/download_file/" . $data->id), "<i data-feather='download-cloud' class='icon-16'></i>", array("title" => app_lang("download")));

        if ($this->login_user->user_type == "staff") {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("agents/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function view_file($file_id = 0) {
        ...
        if ($file_info) {
            ...
            return $this->template->view("agents/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function contact_profile($contact_id = 0, $tab = "") {
        ...
        if ($view_data['user_info']->user_type === "client") {
            ...
            return $this->template->rander("agents/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function my_preferences() {
        ...
        return $this->template->view("agents/contacts/my_preferences", $view_data);
    }
    ...
    function contacts($Sections_id = 0) {
        ...
        return $this->template->view("agents/contacts/index", $view_data);
    }
    ...
    function add_new_contact_modal_form() {
        ...
        return $this->template->view('agents/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            return $this->template->view('agents/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    function company_info_tab($Sections_id = 0) {
        if ($Sections_id) {
            ...
            return $this->template->view('agents/contacts/company_info_tab', $view_data);
        }
    }
    ...
    function save_contact() {
        ...
        $primary_contact = $this->Agents_model->get_primary_contact($Sections_id);
        if (!$primary_contact) {
    ...
    private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false) {
        ...
        $contact_link = anchor(get_uri("agents/contact_profile/" . $data->id), $full_name . $primary_contact) . $removal_request_pending;
        ...
        $row_data = array(
            $user_avatar,
            $contact_link,
            anchor(get_uri("agents/view/" . $data->Sections_id), $client_info->company_name),
            $data->job_title,
            $data->email,
        ...
        $row_data[] = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("agents/delete_contact"), "data-action" => "delete"));
    ...
    function invitation_modal() {
        ...
        $view_data["client_info"] = $this->Clients_model->get_one($client_id);
        return $this->template->view('agents/contacts/invitation_modal', $view_data);
    }
    ...
    function users() {
        if ($this->login_user->user_type === "client") {
            ...
            return $this->template->rander("agents/contacts/users", $view_data);
        }
    }
    ...
    function import_clients_modal_form() {
        ...
        return $this->template->view("agents/import_clients_modal_form");
    }
    ...
    function gdpr() {
        $view_data["user_info"] = $this->Users_model->get_one($this->login_user->id);
        return $this->template->view("agents/contacts/gdpr", $view_data);
    }
    ...
    function request_my_account_removal() {
        if (get_setting("enable_gdpr") && get_setting("clients_can_request_account_removal")) {
            ...
            app_redirect("agents/contact_profile/$user_id/gdpr");
        }
    }
    ...
    function expenses($client_id) {
        ...
        if ($client_id) {
            ...
            return $this->template->view("agents/expenses/index", $view_data);
        }
    }
    ...
    function contracts($client_id) {
        ...
        if ($client_id) {
            ...
            return $this->template->view("agents/contracts/contracts", $view_data);
        }
    }
    ...
    function properties_list() {
        ...
        return $this->template->view("agents/properties_list", $view_data);
    }
    ...
    function proposals($client_id) {
        ...
        if ($client_id) {
            ...
            return $this->template->view("agents/proposals/proposals", $view_data);
        }
    }
    ...
    function tasks($client_id) {
        ...
        $view_data['client_id'] = clean_data($client_id);
        return $this->template->view("agents/tasks/index", $view_data);
    }
}
