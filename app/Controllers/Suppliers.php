...
class Suppliers extends Security_Controller {
    ...
    function save() {
        ...
        $save_id = $this->Suppliers_model->ci_save($supplier_data, $id);
        if ($save_id) {
            $data = $this->Suppliers_model->get_details(['id' => $save_id])->getRow();
            echo json_encode(array("success" => true, "id" => $save_id, "data" => $this->_make_supplier_row($data), 'message' => app_lang('record_saved')));
        } else {
    ...
    function delete() {
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Suppliers_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Suppliers_model->get_details($options)->getRow();
                echo json_encode(array("success" => true, "id" => $item_info->id, "data" => $this->_make_supplier_row($item_info), "message" => app_lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, app_lang('error_occurred')));
            }
        } else {
            if ($this->Suppliers_model->delete($id)) {
                $item_info = $this->Suppliers_model->get_one($id);
    ...
    function list_data() {
        ...
        $list_data = $this->Suppliers_model->get_details()->getResult();
        $result = array();
    ...
    private function _make_supplier_row($data) {
        ...
        return array(
            ...
            $data->email,
            $data->remarks,
            modal_anchor(get_uri("suppliers/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("suppliers/delete"), "data-action" => "delete"))
        );
    }
    ...
    function view() {
        ...
        return $this->template->view('suppliers/view', $view_data);
    }
    ...
    function import_items_modal_form() {
        ...
        return $this->template->view("suppliers/import_items_modal_form");
    }
