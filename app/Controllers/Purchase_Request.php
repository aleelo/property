...
class Purchase_Request extends Security_Controller
{
    ...
    //    $res = $this->check_access('lead');
       ...
       $view_data["can_add_template"] = $can_add_template;
        ...
        return $this->template->rander("purchase_request/index", $view_data);
    }
    
     /* load receive add modal */
     public function request_modal_form()
     {
        ...
        $view_data['request_details'] = $this->Purchase_Request_model->get_request_items(['purchase_request_id' => $id])?->getResult();
        ...
        if($id){
            ...
            $options = array('id' => $id);
            $view_data['model_info'] = $this->Purchase_Request_model->get_details($options)->getRow(); //$this->Subscriptions_model->get_one($lead_id);//
        }else{
            $view_data['model_info'] = new $this->Purchase_Request_model;
        }
        // print_r($view_data['model_info']);die;
               
 
         return $this->template->view('purchase_request/request_modal_form', $view_data);
     }
    
     /* load receive add modal */
     public function delivery_note_modal($id)
     {

        // $view_data['orders_dropdown'] = $this->get_purchase_orders_for_receive(['status' => 'Pending']);
        ...
        // print_r($view_data['model_info']);die;
         ...
         return $this->template->view('purchase_request/delivery_note_modal', $view_data);
     }
     ...
     function save_delivery_note(){
         ...
         $data = array(
             ...
             'delivered_by' => $this->request->getPost('delivered_by'),
         ...
         if($id){
             ...
             $save_id = $this->Purchase_Request_model->ci_save($data, $id);
             echo json_encode(array("success" => true, 'id' => $save_id, 'message' => 'Delivery Note Saved'));
         }else{
            echo json_encode(array("success" => false, 'message' => 'Error Occured!,try again.'));
     ...
     public function request_edit_modal_form()
     {
        ...
        // print_r($view_data['model_info']);die;
         ...
         return $this->template->view('purchase_request/request_edit_modal_form', $view_data);
     }
    ...
    function request_details_json($id=0) {
        
        $request_id = $this->request->getPost('request_id');
        $id = $id == 0 ? $request_id : $id;

        // die('id: ' .$id);
     
        if($id){

            $res = $this->Purchase_Request_model->get_details(['id' => $id])?->getRow();
        }else{
            $res = [];
        }
    ...
    function request_items_json($id=0) {
        ...
        if($id){
            ...
            $res = $this->Purchase_Request_model->get_request_items(['purchase_request_id' => $id])?->getResult();;
        }else{
            $res = [];
        }
        // if (!$res) {
    ...
    function request_details() {
        ...
        $id = $this->request->getPost('id');
        $model_info = $this->Purchase_Request_model->get_details(['id' => $id])->getRow();
        ...
        return $this->template->view("purchase_request/request_details", $view_data);
    }
    ...
    public function save()
    {
        ...
        $save_id = $this->Purchase_Request_model->ci_save($input, $id);
        ...
        if ($save_id) {
            ...
            if (!$id) { //create operation
                ...
                log_notification("purchase_request_created", array("purchase_request_id" => $save_id), $this->login_user->id);
                ...
                log_notification("purchase_request_updated", array("purchase_request_id" => $id), $this->login_user->id);
    ...
    function item_modal_form() {
        $purchase_request_id = $this->request->getPost('purchase_request_id');
        ...
        $view_data['model_info'] = $this->Purchase_Request_Items_model->get_one($id);

        if (!$purchase_request_id) {
            $purchase_order_id = $view_data['model_info']->purchase_request_id;
        }

        $view_data['purchase_request_id'] = $purchase_request_id;
        return $this->template->view('purchase_request/item_modal_form', $view_data);
    }
     ...
     function get_purchase_item_suggestion() {
        ...
        $items = $this->Purchase_Request_Items_model->get_item_suggestion($key);
    ...
    function get_purchase_item_info_suggestion() {
        $item = $this->Purchase_Request_Items_model->get_item_info_suggestion(array("item_id" => $this->request->getPost("item_id")));
        if ($item) {
            $item->price = $item->price ? to_decimal_format($item->price) : "";
     ...
     private function _get_purchase_total_view($purchase_id = 0) {
        $view_data["purchase_total_summary"] = $this->Purchase_Request_Items_model->get_purchase_total_summary($purchase_id);
        $view_data["purchase_id"] = $purchase_id;        

        $can_edit_purchase = true;
            
        $view_data["can_edit_purchase"] = $can_edit_purchase;
        return $this->template->view('purchase_request/purchase_total_section', $view_data);
    }
    ...
    function save_item() {
        ...
        // $purchase_item_id = $this->Purchase_Request_Items_model->save_item_and_update_invoice($purchase_item_data, $id, $purchase_request_id);
        $purchase_item_id = $this->Purchase_Request_Items_model->ci_save($purchase_item_data,$id);
        if ($purchase_item_id) {
            $options = array("id" => $purchase_item_id);
            $purchase_item_info = $this->Purchase_Request_Items_model->get_details($options)->getRow();
            echo json_encode(array("success" => true, "purchase_request_id" => $purchase_item_info->purchase_request_id, "data" => $this->_make_item_row($purchase_item_info, true), "invoice_total_view" => $this->_get_purchase_total_view($purchase_item_info->purchase_request_id), 'id' => $purchase_item_id, 'message' => app_lang('record_saved')));
        } else {
    ...
    function download_pdf($id = 0, $mode = "download", $user_language = "") {
        ...
        if ($id) {
            ...
            $data = array();
            $purchase_info = $this->Purchase_Request_model->get_details(array("id" => $id))->getRow();
            ...
            $data["purchase_total_summary"] = $this->Purchase_Request_model->get_purchase_total_summary($id);
   ...
   public function get_requisition_order_document($uuid){

       $purchase_info = $this->Purchase_Request_model->get_details(['uuid' => $uuid])->getRow();      
       $id = $purchase_info?->id;
       $items_info = $this->Purchase_Request_model->get_request_items(['purchase_request_id' => $id])->getResult();

       //get or generate document:
       $request_doc =  $this->db->query("SELECT d.* FROM rise_documents d
                    left join rise_purchase_request_document rd on d.id = rd.document_id where rd.request_id = $id")->getRow();
       
       if(!$request_doc){
        
        echo 'Generating New Document for first time...';
       $arr_table = [];
       $index = 0;
       ...
       $doc_request_data = [
           'id'=>$id,
           'uuid'=>$purchase_info->uuid,
           'ref_number'=> $template->ref_prefix.'/'.$sqn.'/'.date('m').'/'.date('Y'),
           'template' => $template->path,
           'folder' => $template->destination_folder,
           'date' => date('Y-m-d'),
           'prid' => $this->get_PR_ID($id),
           'requested_by' => $purchase_info->user,
           'table' => $arr_table,
           "department" => $this->get_department_name($purchase_info->department_id),
       ...
       $doc_data = [
           ...
           'document_title' =>'Requisition Document - '.$purchase_info->id,
           'ref_number' =>$template->ref_prefix.'/'.$sqn.'/'.date('m').'/'.date('Y'),
       ...
       $this->db->query("insert into rise_purchase_request_document(request_id,document_id) values($id,$doc_id)");
       $doc_request_data['document_id'] = $doc->id;

       $path = $this->Purchase_Request_model->createDoc($doc_request_data);
       $token = $this->Purchase_Request_model->AccesToken();  
       $data = $this->Purchase_Request_model->uploadDoc($token,$doc_request_data,$path);   
    ...
    function print_invoice($purchase_id = 0) {
        if ($invoice_id) {
            validate_numeric_value($purchase_id);
            $purchase_request = $this->Purchase_Request_model->get_details(['id' => $purchase_id])->getRow();
    ...
    function delete_item() {
        ...
        $id = $this->request->getPost('id');
        $item_info = $this->Purchase_Request_Items_model->get_one($id);
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Purchase_Request_Items_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Purchase_Request_Items_model->get_details($options)->getRow();
                echo json_encode(array("success" => true, "invoice_id" => $item_info->purchase_request_id, "data" => $this->_make_item_row($item_info, true), "invoice_total_view" => $this->_get_purchase_total_view($item_info->purchase_request_id), "message" => app_lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, app_lang('error_occurred')));
            }

        } else {
            if ($this->Purchase_Request_Items_model->delete($id)) {
                $item_info = $this->Purchase_Request_Items_model->get_one($id);
                echo json_encode(array("success" => true, "invoice_id" => $item_info->purchase_request_id, "invoice_total_view" => $this->_get_purchase_total_view($item_info->purchase_request_id), 'message' => app_lang('record_deleted')));
            } else {
    ...
    function item_list_data($purchase_order_id = 0) {
        ...
        $list_data = $this->Purchase_Request_Items_model->get_details(array("purchase_request_id" => $purchase_order_id))->getResult();
    ...
    private function _make_item_row($data, $is_ediable) {
        ...
        if ($data->description) {
            $desc = "<span class='text-wrap' $desc_style>" . nl2br($data->description) . "</span>";
        }
        ...
        return array(
            ...
            modal_anchor(get_uri("purchase_request/item_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_purchase'), "data-post-id" => $data->id, "data-post-purchase_request_id" => $data->purchase_request_id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("purchase_request/delete_item"), "data-action" => "delete"))
        );
    }


    function request_details_tab($purchase_request_id) {
                // die($purchase_order_id);
        $model_info = $this->Purchase_Request_model->get_details(['id' => $purchase_request_id])->getRow();

        if (!$model_info) {
            show_404();
        }
        
        $view_data["purchase_total_summary"] = $this->Purchase_Request_Items_model->get_purchase_total_summary($purchase_request_id);

        $view_data['purchase_info'] = $model_info;
        $view_data['purchase_request_ids'] = $purchase_request_id;
        $view_data['can_add_purchase'] = true;
        $view_data['can_edit_purchase'] = true;

        return $this->template->view("purchase_request/request_details_tab", $view_data);
    }

    function view($purchase_request_id = 0) {
        // die($purchase_order_id);
        if (!$purchase_request_id) {
            show_404();
        }

        // print_r($purchase_request_id);die;
        if ($purchase_request_id) {;
            $order_options = array('purchase_request_id' => $purchase_request_id);

            $purchase_info = $this->Purchase_Request_model->get_details(['id' => $purchase_request_id])->getRow();

            $view_data['purchase_info'] = $purchase_info;
            $view_data['purchase_request_items'] = $this->Purchase_Request_model->get_request_items($order_options)->getResult();

                $view_data['purchase_status'] = $purchase_info->status;
                // $view_data['purchase_order_id'] = $purchase_order_id;
                $view_data["can_add_purchase"] = true;
                return $this->template->rander("purchase_request/view_purchase", $view_data);
        } else {
            show_404();
        }
    ...
    public function delete()
    {
        ...
        $r = $this->Purchase_Request_model->get_one($id);
        $request_id = $r?->request_id;

        if ($this->Purchase_Request_model->delete($id)) {
            $this->db->query("DELETE FROM rise_purchase_request_items WHERE purchase_request_id = $request_id");
    ...
    public function request_list_data()
    {
        ...
        if (get_array_value($options, "server_side")) {
            ...
            $result = $this->db->query("select rc.*,dp.nameSo as department,concat(u.first_name,' ',u.last_name) user,s.supplier_name as supplier from rise_purchase_requests rc 
            LEFT JOIN rise_users u on rc.requested_by = u.id 
            LEFT JOIN rise_departments dp on rc.department_id = dp.id 
            ...
            $list_data = $result->getResult();
            $total_rows =$this->db->query("select count(*) as affected from rise_purchase_requests rc
        ...
        $result_data = array();
        foreach ($list_data as $data) {
            $result_data[] = $this->_make_request_row($data);
        }
    ...
    private function request_row_data($id)
    {
        ...
        $data = $this->Purchase_Request_model->get_details($options)->getRow();

        return $this->_make_request_row($data, $custom_fields);
    }
    
    private function _make_request_row($data)
    {
        ...
        }else if (strtolower($data->status) === "delivered") {
            $status_class = "btn-success";//btn-success
        } else if (strtolower($data->status) === "submitted") {
            $status_class = "bg-success";//btn-success
        ...
        $requestDate = empty($data->request_date)? '' : date_format(new \DateTime($data->request_date),'F d, Y');
        $row_data = array(
            $data->id,
            anchor(get_uri("purchase_request/view/".$data->id), $this->get_PR_ID($data->id), array("title" => app_lang("purchase_details"), "data-post-id" => $data->id)),
        ...
        $row_data[] = modal_anchor(get_uri("purchase_request/request_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => 'Edit Order Information', "data-post-id" => $data->id))
        .modal_anchor(get_uri("purchase_request/request_details"), "<i data-feather='info' class='icon-16'></i>", array("class" => "info", "title" => 'Show Order Information', "data-post-id" => $data->id))
        . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => 'Delete Order Information', "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("purchase_request/delete"), "data-action" => "delete-confirmation"));
